...
//Nestes endpoint que recebem 2 ids , supondo id_colaborador , id_projeto e id_equipe, se não podemos fazer um zod que receba primeiro_id e segundo_id. Assim podemos aplicar ele em varios endpoints.
//depois separar projeto por pastas
...
const opcoesCargo = [
    "Backend", "Frontend", "Marketing", "Contador", "Financeiro", "Vendedor"
]
...
const BodyCadastroTarefaSchema = z.object({
    ...
    titulo: z.string({
        ...
        .min(1, { message: "Não pode ter menos de 1 caractere" })
        .max(20, { message: "Não pode ter mais de 20 caracteres" }),
    status: z.enum(opcoesStatus, { message: "Status inválido" })
        .optional(),
    prioridade: z.enum(opcoesPrioridades, { message: "Prioridade inválida" })
        .optional(),
    data_termino: z.string({ message: "Campo obrigatório" })
        .regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Data inválida" }),
    descricao: z.string({
...
app.post('/tarefa/cadastrar', async (req, res) => {
    try {
        ...
        const [row] = await db.query(
            "select Alocacoes.id_equipe from Membros inner join Alocacoes on Membros.id_equipe = Alocacoes.id_equipe where id_projeto= ? and id_colaborador= ?"
...
const BodyCadastroMembroSchema = z.object({
    ...
    id_equipe: z.number({
        ...
        .min(1, { message: "Id tem que ter pelo menos 1 digito" }),
    cargo: z.enum(opcoesCargo, { message: "Cargo inválido" }),
})
...
const BodyCadastroAlocacaoSchema = z.object({
    id_equipe: z.number({
        ...
        .min(1, { message: "Id tem que ser maior que 0" }),
    id_projeto: z.number({
        invalid_type_error: "Id precisa ser de tipo numérico",
        required_error: "Campo obrigatório"
    })
...
app.get('/projeto/consultar/cargos', async (req, res) => {
    try {
        const verificacaoQuery = QueryConsultaIdSchema.safeParse(req.query)
        if (verificacaoQuery.success == false) return res.status(422).json({ error: verificacaoQuery.error.format() });
        const { id } = req.query;
        const [row] = await db.query('select Membros.cargo from Alocacoes inner join Membros on Membros.id_equipe = Alocacoes.id_equipe where Alocacoes.id_projeto= ? ', [id])
        if (row.length < 1) {
            // Status: Not Found
            return res.status(404).json(`Não tem nenhum cargo neste projeto.`);
        }
        return res.status(200).json(row);
    }
    catch (e) {
...
//select Tarefas.titulo as "Tarefas", Colaboradores.nome as "Colaboradores", Tarefas.status from Tarefas
//inner join Colaboradores on Colaboradores.id = Tarefas.id_colaborador 
//where id_projeto = ?;
...
//inner join Alocacoes on Membros.id_equipe = Alocacoes.id_equipe 
...
//5-Consultar cargos por projeto
//select Membros.cargo from Alocacoes 
//inner join Membros on Membros.id_equipe = Alocacoes.id_equipe 
//where Alocacoes.id_projeto= ? 
